...
class _BFNode:
    ...
    def trim_to_k_subclusters(self, currK, targetK, targetLevel):
        if self is None or currK[0]==targetK:
            return currK

        for i in self.subclusters_:
            if targetLevel!=0 and targetLevel!=1:
                i.child_.trim_to_k_subclusters(currK, targetK, targetLevel-1)
            else:
                child_node=i.child_
                if targetLevel==0:
                    child_node=self
                #print(len(child_node.centroids_))
                
                if len(child_node.centroids_)>1:
                    pop_counts = np.sum(child_node.centroids_, axis=1)
                    pop_mols = np.sum(child_node.centroids_, axis=1).reshape(-1,1)
                    ei = np.einsum("ij,kj->ik", child_node.centroids_, child_node.centroids_)
                    ...
                    child_node.subclusters_[index[0]].update(child_node.subclusters_[index[1]])

                    child_node.centroids_[index[0]]=child_node.subclusters_[index[0]].centroid_
                    child_node.init_centroids_[index[0]]=child_node.subclusters_[index[0]].centroid_
                    
                    # delete necessary
                    del child_node.subclusters_[index[1]]
                    child_node.centroids_=np.delete(child_node.centroids_, index[1], axis=0)
                    child_node.init_centroids_=np.delete(child_node.init_centroids_, index[1], axis=0)
                ...
                for j in child_node.subclusters_:
                    j.child_=None                
...
#Refinement functionality 
        ...
        def link_leaves(node, prev_leaf):
            ...
            for i in node.subclusters_:
                if not i.child_.is_leaf:
                    ...
                    '''
                    if prev_leaf[0]==-1:
                        self.dummy_leaf_.next_leaf_=i.child_
                    else:
                        prev_leaf[0].next_leaf=i.child_
                    prev_leaf[0]=i.child_
                    '''
    ...
    def find_level_k(self):
        levels=[]
        centroids=[]
        targetLevel=0
        node=self.root_

        self.recursively_traverse(node, levels, centroids, targetLevel)
        ...
        while currK[0]>targetK:
            #pdb.set_trace()
            currK=self.root_.trim_to_k_subclusters(currK, targetK, targetLevel)
            if prevK==currK[0]:
                print(f"trimmed as many clusters ({prevK})as we could; input a smaller k for full results")
                break
            prevK=currK[0]
        ...
        # to merge across nodes and im not sure how that would be done efficiently
