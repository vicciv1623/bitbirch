...
class _BFNode:
    ...
    def insert_leaf_subcluster(self, subcluster, set_bits, merge):
        if not self.subclusters_:
            self.append_subcluster(subcluster)
            return False
        ...
        a = np.dot(self.centroids_, subcluster.centroid_)
        #print(set_bits)
        #print(self.centroids_.shape," ",subcluster.centroid_.shape)
        sim_matrix = a / (np.sum(self.centroids_, axis = 1) + set_bits - a)
        closest_index = np.argmax(sim_matrix)
        closest_subcluster = self.subclusters_[closest_index]

        if closest_subcluster.child_ is not None:
            split_child = closest_subcluster.child_.insert_leaf_subcluster(subcluster, set_bits, merge)
            ...
            # subcluster to accommodate the new child.
                ...
                self.update_split_subclusters(
                    closest_subcluster, new_subcluster1, new_subcluster2, True
                )
        ...
        # good to go!
            ...
            # when building tree of top k largest leafs, we do not want to merge since we want them to 
            # be the k leaves, therefore we will only append and split if necessary
...
class BitBirch():
    ...
    def fit_leaves(self, top_k_clusters, not_k_clusters, X):
        threshold=self.threshold
        ...
        for top_cluster in top_k_clusters:
...
#Refinement functionality 
    ...
    def trimClusters(self):
        # current method implemented is to find similar clusters within each leaf node and merge
        leaves=self._get_leaves()
        currK=len(self.get_centroids())

        while currK>self.k:
            for i in leaves:
                if len(i.centroids_)==1:
                    continue
                pop_counts = np.sum(i.centroids_, axis=1)
                pop_mols = np.sum(i.centroids_, axis=1).reshape(-1,1)
                ei = np.einsum("ij,kj->ik", i.centroids_, i.centroids_)
                ...
                #merging between two most similar subclusters
                i.subclusters_[index[0]].update(i.subclusters_[index[1]])

                i.centroids_[index[0]]=i.subclusters_[index[0]].centroid_
                i.init_centroids_[index[0]]=i.subclusters_[index[0]].centroid_
                
                # delete necessary
                del i.subclusters_[index[1]]
                i.centroids_=np.delete(i.centroids_, index[1], axis=0)
                i.init_centroids_=np.delete(i.init_centroids_, index[1], axis=0) 

                currK=len(self.get_centroids())
                if currK==self.k:
                    break

    def trimClusters2(self, X):
        # find top k largest clusters -> build new tree (k leaves) -> insert rest of leafs
        cluster_sizes=[]
        clusters=[]
        ind=0
        ...
        for i in cluster_sizes:
            if len(top_k_sizes)<self.k:
                heapq.heappush(top_k_sizes, (i[0], i[1]))
            else:
                heapq.heappushpop(top_k_sizes, (i[0], i[1]))
                    
        top_k_ind=[ind for size, ind in top_k_sizes]
        #print(top_k_sizes)
        ...
        for ind, i in enumerate(clusters):
            if ind in top_k_ind:
                top_k_clusters.append(i)
            else:
                not_k_clusters.append(i)
                
        self.fit_leaves(top_k_clusters, not_k_clusters, X)
