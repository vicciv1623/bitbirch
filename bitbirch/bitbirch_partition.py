...
class _BFSubcluster:
    ...
    def noise_criteria(self, closest_cluster, leaf_centroids, merge, tol, threshold):
        a = np.dot(leaf_centroids, self.centroid_)
        q = 1 - a / (np.sum(leaf_centroids, axis=1) + np.sum(self.centroid_) - a)
        p = tol
        r = 1-threshold
        n = q <= r + p
        if np.sum(n) > 1:
            ...
            i = leaf_centroids[n==1]
            is_closest = (i == closest_cluster.centroid_).all()
            if is_closest:
                return False if merge else True
            else:
                return True 
